...
export const Chat = () => {
    ...
    useEffect(() => {
        ...
        socketConnection.on('mensaje', (mensaje) => {
            ...
            if (!currentUser) {
                console.error(
                    'No se puede recibir mensajes si no hay un usuario logueado'
                );
                return;
            }
            ...
            if (selectedConversation) {
                try {
                    const response = await fetch(
                        `${API}/mensajes/${selectedConversation.conversacion_id}`,
                        {
                            method: 'GET',
                            headers: {
    ...
    const handleChange = async (e) => {
        ...
        try {
            const fetchUsers = await fetch(`${API}/users/chat/${value}`, {
                method: 'GET',
                headers: {
                    authorization: `${localStorage.getItem('AUTH_TOKEN')}`,
                },
    ...
    const handleSubmit = (e) => {
        ...
        if (message.trim() !== '' && socket) {
            ...
            if (!idDestinatario) {
                console.error(
                    'No se puede enviar mensajes si no hay un destinatario'
                );
                return;
            }
            ...
            <main className="mx-auto max-w-6xl flex h-screen">
                <div className="w-2/4 flex flex-col gap-2 mx-4 bg-[#93F]/40 border rounded h-3/4 my-10 px-8">
                    ...
                    <input
                        ...
                        className="form-input self-start m-0"
                    />
                        ...
                        <ul className="list-none my-4">
                            {searchedUsers.map((user) => (
                                <li
                                    ...
                                    onClick={() =>
                                        handleIniciateConversacion(user.id)
                                    }
                                >
                                    {user.avatar ? (
                                        <img
                                        ...
                                        <>
                                            {conversacion.usuario2_avatar ? (
                                                <img
                                                    ...
                                                    alt={
                                                        conversacion.usuario2_username
                                                    }
                                                    className="w-10 h-10 rounded-full"
                                                />
                                            ) : (
                                        ...
                                        <>
                                            {conversacion.usuario1_avatar ? (
                                                <img
                                                    ...
                                                    alt={
                                                        conversacion.usuario1_username
                                                    }
                                                    className="w-10 h-10 rounded-full"
                                                />
                                            ) : (
                    ...
                    <div className="border flex-grow-0 flex place-items-center gap-4 p-4">
                        {selectedConversation ? (
                            <h2 className="text-2xl">
                                Chat con{' '}
                                {selectedConversation.usuario1_id !==
                                currentUser.id
                                    ? selectedConversation.usuario1_username
                                    : selectedConversation.usuario2_username}
                            </h2>
                        ) : (
                            <h2 className="text-2xl">
                        ...
                        {selectedConversation ? (
                            messages.map((message, index) => {
                                ...
                                if (
                                    ...
                                    message.mensaje === lastSentMessage
                                ) {
                    ...
                    {selectedConversation && (
                        <div className="flex-0">
                            <form
                                ...
                                className="flex gap-2 bottom-2 left-0 right-0 max-w-4xl mx-auto px-4"
                            >
                                <input
                                    type="text"
                                    name="message"
