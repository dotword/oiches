...
console.log(formValues);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.success('Reserva creada con éxito espere a que la sala confirme su reserva.');
    } catch (error) {
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 mx-auto max-w-6xl">
        <h2 className="text-2xl">Elija los datos para la reserva :</h2>
        <label htmlFor="fecha">
          Elija una fecha para la reserva:
          <input className="block" type="date" name="fecha" required />
        </label>
        <label htmlFor="horaInicio">
          Elija una hora para el inicio de la reserva:
          <input className="block" type="time" name="horaInicio" required />
        </label>
        <label htmlFor="horaFin">
          Elija una hora aproximada para el fin de la reserva:
          <input className="block" type="time" name="horaFin" required />
        </label>
        <div className="flex justify-evenly">
          <button className="button" type="submit">Crear Reserva</button>
