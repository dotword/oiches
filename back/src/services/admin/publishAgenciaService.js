...
const publishAgenciaService = async (idAgencia) => {
    try {
        const pool = await getPool();

        const [entryInfo] = await pool.query(
            'SELECT published, usuario_id, nombre FROM agencias WHERE id = ?',
            [idAgencia]
        );
        ...
        if (entryPublished === 1) {
            throw generateErrorsUtil('La agencia ya está publicada', 404);
        }

        // Comprobar el email y nombre de la agencia
        const [emailAgencia] = await pool.query(
            'SELECT email FROM usuarios WHERE id = ?',
            [user_id]
        );
        const agenciaEmail = emailAgencia[0].email;

        // Creamos el asunto del email de verificación.
        const emailSubject = `${entryName} ya está publicada en Oiches`;
        ...
        const emailBody = `
                ...
                <body>
                    ...
                    <p>¡Te informamos de que ${entryName} ya está publicada en Oiches!</p>

                    <p>Entra en tu cuenta en el siguiente enlace: <b><a href="${URL_FRONT}/login">Mi cuenta</a></b>, para publicar el roster de todos tus artistas y empezar a conectar con salas para programar los próximos conciertos de tus músicos.</p>
  ...
  <p>--</p>
    ...
    <p class="small-text"><strong>AVISO SOBRE CONFIDENCIALIDAD:</strong> Esta comunicación contiene información que es confidencial y también puede contener información privilegiada. Es para uso exclusivo del destinatario/s. Si usted no es el destinatario/s tenga en cuenta que cualquier distribución, copia o uso de esta comunicación o la información que contiene está estrictamente prohibida. Si usted ha recibido esta comunicación por error por favor notifíquelo a <a href="mailto:hola@oiches.com">hola@oiches.com</a>.</p>
        ...
        try {
            await sendMailUtil(agenciaEmail, emailSubject, emailBody);
        } catch (error) {
            return;
        }

        await pool.query('UPDATE agencias SET published = 1 WHERE id = ?', [
            idAgencia,
        ]);
    } catch (error) {
        console.log(error);
...
export default publishAgenciaService;
