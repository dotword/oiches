...
const publishSalaService = async (salaId) => {
    try {
        const pool = await getPool();

        const [entryInfo] = await pool.query(
            'SELECT published, usuario_id, nombre FROM salas WHERE id = ?',
            [salaId]
        );
        ...
        // Comprobar que la sala no esté publicada
        if (entryPublished === 1) {
            throw generateErrorsUtil('La sala ya está publicada', 404);
        }

        // Comprobar el email y nombre de la sala
        ...
        const emailBody = `
                ...
                <body>
                    ...
                    <p>Entra en tu cuenta en el siguiente enlace: <b><a href="${URL_FRONT}/login">Mi cuenta</a></b>, para terminar de completar tu perfil y empezar a gestionar tu calendario y conectar con artistas para tus próximos conciertos.</p>
  ...
  <p>--</p>
    ...
    <p class="small-text"><strong>AVISO SOBRE CONFIDENCIALIDAD:</strong> Esta comunicación contiene información que es confidencial y también puede contener información privilegiada. Es para uso exclusivo del destinatario/s. Si usted no es el destinatario/s tenga en cuenta que cualquier distribución, copia o uso de esta comunicación o la información que contiene está estrictamente prohibida. Si usted ha recibido esta comunicación por error por favor notifíquelo a <a href="mailto:hola@oiches.com">hola@oiches.com</a>.</p>
        ...
        await pool.query('UPDATE salas SET published = 1 WHERE id = ?', [
            salaId,
        ]);
    } catch (error) {
        console.log(error);
...
export default publishSalaService;
