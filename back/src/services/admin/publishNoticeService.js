...
const publishNoticeService = async (idNotice) => {
    try {
        const pool = await getPool();

        const [noticeInfo] = await pool.query(
            'SELECT estado, usuario_id, titulo FROM noticeboard WHERE id = ?',
            [idNotice]
        );

        const noticePublished = noticeInfo[0].published;
        const user_id = noticeInfo[0].usuario_id;
        const noticeTitle = noticeInfo[0].titulo;

        // Comprobar que la notice no esté publicada
        if (noticePublished === 'aprobado') {
            throw generateErrorsUtil('El anuncio ya está publicada', 404);
        }
        ...
        const emailSubject = `Tu anuncio "${noticeTitle}" ya está publicada en Oiches`;
        ...
        const emailBody = `
                ...
                <body>
                    ...
                    <p>¡Te informamos de que tu anuncio "${noticeTitle}" ya está publicada en Oiches!</p>

                    <p>Si quieres eliminarlo, puedes hacerlo desde el panel de control de tu cuenta: <b><a href="${URL_FRONT}/login">Mi cuenta</a></b>.</p>
  ...
  <p>--</p>
    ...
    <p class="small-text"><strong>AVISO SOBRE CONFIDENCIALIDAD:</strong> Esta comunicación contiene información que es confidencial y también puede contener información privilegiada. Es para uso exclusivo del destinatario/s. Si usted no es el destinatario/s tenga en cuenta que cualquier distribución, copia o uso de esta comunicación o la información que contiene está estrictamente prohibida. Si usted ha recibido esta comunicación por error por favor notifíquelo a <a href="mailto:hola@oiches.com">hola@oiches.com</a>.</p>
        ...
        try {
            await sendMailUtil(userEmail, emailSubject, emailBody);
        } catch (error) {
            return;
        }

        await pool.query(
            'UPDATE noticeboard SET estado = "aprobado" WHERE id = ?',
            [idNotice]
        );
    } catch (error) {
        console.log(error);
...
export default publishNoticeService;
