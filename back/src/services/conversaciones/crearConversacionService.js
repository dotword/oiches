...
export const crearConversacionService = async (idUsuario, idUsuarioDestino) => {
  ...
  const [usuario1Rows] = await pool.query(`SELECT username, avatar, email FROM usuarios WHERE id = ?`, [idUsuario]);
  const [usuario2Rows] = await pool.query(`SELECT username, avatar, email FROM usuarios WHERE id = ?`, [idUsuarioDestino]);
  ...
  // Log para verificar la información recuperada
