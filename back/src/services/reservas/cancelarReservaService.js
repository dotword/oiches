...
try {
  ...
  if (grupoResults.length === 0) {
    throw {
      status: 404,
      message: "No se han encontrado grupos con el usuario con el que esta intentado acceder."
    };
  }
  const grupo_id = grupoResults[0].id;
  
  const [reservaResults] = await pool.query('SELECT * FROM Reservas WHERE grupo_id = ? AND sala_id = ?',[grupo_id,sala_id])
