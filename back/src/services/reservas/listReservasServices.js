...
export const listReservaService = async (id) => {
  ...
  try {
    ...
    const [salaResults] = await pool.query('SELECT id,nombre FROM salas WHERE usuario_id = ?', [id]);
    ...
    const [reservas] = await pool.query( `SELECT reservas.*, grupos.nombre AS grupo_nombre, salas.nombre AS sala_nombre
    FROM reservas
    LEFT JOIN grupos ON reservas.grupo_id = grupos.id
    LEFT JOIN salas ON reservas.sala_id = salas.id
    WHERE reservas.sala_id IN (?)`, [salasId]);
