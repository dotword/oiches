...
export const deleteGrupoService = async (grupoId)=>{
  try {
   ...
   console.log(grupoId);
         ...
         await pool.query(`DELETE FROM grupo_media WHERE grupo_id = ?`, [grupoId]);
         await pool.query(`DELETE FROM grupo_fotos WHERE grupoId = ?`, [grupoId]);
   
         // Seleccionar reservas del grupo
         const [reservas] = await pool.query(`SELECT id FROM reservas WHERE grupo_id = ?`, [grupoId]);
       ...
       return {message :"Grupo y todas sus relaciones eliminadas exitosamente"}
  } catch (error) {
    console.log(error);
    throw error
  }
